...
int client_proxy::parse_config_power_stream_data(const DataXmlVec& vec, UnionSimDatVec& data)
{
    ...
    case eInteract_ctrl_to_power:{
        ...
        for(int i=0; i<count; ++i){
            ...
            data[i].sim_time = grid.PowerUpadateTime;
            IntPairMap::const_iterator it = _dw_comm_id_tbl.find(grid.DG_node_ID);
            if(it != _dw_comm_id_tbl.cend()){
                const IntPair& pr = it->second;
                data[i].comm_dat.src_id =pr.first;
                data[i].comm_dat.dst_id = pr.second;
            }
...
void client_proxy::handle_comm_power_appl(ApplMessage* msg)
{
    ...
    if(proc_type == eSubProcedure_session_begin && msg_type == eMessage_confirm){
        ...
        if(_power_conf_param.prj_type == ePowerPrj_conf_power ){
            ...
            _expect_proc_type = ret > 0 ? eSubProcedure_session_begin : eSubProcedure_session_end;
        }
        else{
