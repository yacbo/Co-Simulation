...
void UploadSamplingFreq(Bus_Info bus,bus_info_upload *bus1)
{
	bus1->PowerSimTime=bus.sampling_time;
	bus1->PowerFrequency=bus.freq;
	bus1->Startnode_ID=0;   //bus1对应通信编号为0
	bus1->Destnode_ID=6;    //牵引节点编号为6
	//----bus1发送至通信软件
}

double DownloadSamplingFreq(Para &para,bus_info_download *bus2){
	//-----bus2由通信软件下发
...
//解析数据包结构体
int packet_prase(commu_to_ctrl tRecv,State &state,Para &para){ 
...
void UploadDGPacket(State &state,Para &para,int node_id,vector<ctrl_to_commu>&tSendDG) {
	ctrl_to_commu a;
	size_t i,j;
...
void DownloadDGPacket(State &state,Para &para,vector<commu_to_ctrl>&tRecvDG){
	//---获得数据包tRecvDG
...
void getDemand(State &state,Para &para,bus_info_upload &bus1,bus_info_download &bus2){
	para.Freq=DownloadSamplingFreq(para,&bus2);  //通信软件向电力应用下发母线信息
	para.P_Demand=setDemand(para,&(state.flag_miu));  //调用控制中心算法计算缺额功率
}

////一次迭代交互
//void Interact(State &state,Para &para,vector<commu_to_ctrl>&tRecvDG,DG_Node dg_node[],Ctrl_to_Grid dg_feedback[]){
...
static bus_info_upload g_upload_bus;
static bus_info_download g_dwload_bus;
...
void generate_upload_sample_freq(const Bus_Info &bus, bus_info_upload& upload_bus_infor)
{
    UploadSamplingFreq(bus, &g_upload_bus);
    memcpy(&upload_bus_infor, &g_upload_bus, sizeof(bus_info_upload));
}

void config_param_state(const bus_info_download& dwload_bus_infor)
{
    memcpy(&g_dwload_bus, &dwload_bus_infor, sizeof(bus_info_download));
    getDemand(g_state, g_para, g_upload_bus, g_dwload_bus);
}

void generate_upload_dgpacket(vector<ctrl_to_commu>& tSendDG)
{
    for(int i=0;i<6;i++){
...
bool iterator_calculate(vector<commu_to_ctrl>& tRecvDG, Ctrl_to_Grid* dg_feedback)
{
    DownloadDGPacket(g_state, g_para, tRecvDG);    //通信软件向电力应用下发数据包
    tRecvDG.clear();
    if(g_state.flag_converge)  //迭代收敛
    {
...
//	vector<ctrl_to_commu>tSendDG;  //电力应用上传至通信软件数据包
//	vector<commu_to_ctrl>tRecvDG;  //通信软件下发至电力应用数据包

//	bus_info_upload bus1;            //电力应用上传通信的母线数据
//	UploadSamplingFreq(bus,&bus1);  //电力软件通过电力算法模块向通信软件上传母线
//	bus_info_download bus2;          //通信软件下发至电力应用的母线数据
